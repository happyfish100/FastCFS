...
int cluster_recv_timeout_callback(struct fast_task_info *task)
{
            ...
            CLUSTER_PEER != NULL)
    {
        ...
        return ETIMEDOUT;
    } else if (SERVER_TASK_TYPE == VOTE_SERVER_TASK_TYPE_SUBSCRIBE &&
...
static void session_subscriber_cleanup(VoteServerContext *server_ctx,
...
void cluster_task_finish_cleanup(struct fast_task_info *task)
{
    switch (SERVER_TASK_TYPE) {
        case VOTE_SERVER_TASK_TYPE_RELATIONSHIP:
            ...
            break;
        case VOTE_SERVER_TASK_TYPE_SUBSCRIBE:
...
void cluster_subscriber_queue_push_ex(ServerSessionSubscriber *subscriber,
        const bool notify)
{
    VoteServerContext *server_ctx;

    if (__sync_bool_compare_and_swap(&subscriber->nio.in_queue, 0, 1)) {
        server_ctx = (VoteServerContext *)subscriber->
...
static int cluster_deal_session_subscribe(struct fast_task_info *task)
{
    ...
    if ((result=server_check_min_body_length(sizeof(
                        FCFSVoteProtoSessionSubscribeReq) + 1)) != 0)
    {
        return result;
    }

    req = (FCFSVoteProtoSessionSubscribeReq *)REQUEST.body;
    ...
    if ((dbuser->user.priv & FCFS_VOTE_USER_PRIV_SUBSCRIBE_SESSION) == 0) {
        RESPONSE.error.length = sprintf(RESPONSE.error.message,
    ...
    SERVER_TASK_TYPE = VOTE_SERVER_TASK_TYPE_SUBSCRIBE;
    RESPONSE.header.cmd = FCFS_VOTE_SERVICE_PROTO_SESSION_SUBSCRIBE_RESP;
    return 0;
}
        ...
        const int64_t pool_id, const int64_t priv_required)
{
    ...
    switch (priv_type) {
        case fcfs_vote_validate_priv_type_user:
            session_priv = (fields.dbuser != NULL) ? fields.dbuser->
                user.priv : (FCFS_VOTE_USER_PRIV_MONITOR_CLUSTER |
                        FCFS_VOTE_USER_PRIV_SUBSCRIBE_SESSION);
            break;
        case fcfs_vote_validate_priv_type_pool_fdir:
            /*
            //TODO
            ...
            */
            ...
            break;
        case fcfs_vote_validate_priv_type_pool_fstore:
...
static int cluster_deal_session_validate(struct fast_task_info *task)
{
    ...
    FCFSVoteProtoSessionValidateResp *resp;
    int64_t session_id;
    int64_t pool_id;
    string_t validate_key;
    int64_t priv_required;
    FCFSVoteValidatePriviledgeType priv_type;
    int result;
    ...
    req = (FCFSVoteProtoSessionValidateReq *)REQUEST.body;
    session_id = buff2long(req->session_id);
    FC_SET_STRING_EX(validate_key, req->validate_key, FCFS_VOTE_PASSWD_LEN);
    ...
    resp = (FCFSVoteProtoSessionValidateResp *)REQUEST.body;
    int2buff(result, resp->result);
    RESPONSE.header.body_len = sizeof(*resp);
    RESPONSE.header.cmd = FCFS_VOTE_SERVICE_PROTO_SESSION_VALIDATE_RESP;
...
static int cluster_process(struct fast_task_info *task)
{
    ...
    if (!MYSELF_IS_MASTER) {
        if (cmd == FCFS_VOTE_SERVICE_PROTO_SESSION_SUBSCRIBE_REQ) {
            RESPONSE.error.length = sprintf(
                    RESPONSE.error.message,
                    "i am not master");
            return SF_RETRIABLE_ERROR_NOT_MASTER;
        ...
        if (cmd == FCFS_VOTE_SERVICE_PROTO_SESSION_VALIDATE_REQ) {
    ...
    switch (cmd) {
        ...
        case FCFS_VOTE_SERVICE_PROTO_SESSION_SUBSCRIBE_REQ:
            return  cluster_deal_session_subscribe(task);
        case FCFS_VOTE_SERVICE_PROTO_SESSION_VALIDATE_REQ:
...
static int cluster_deal_queue(VoteServerContext *server_context,
        ServerSessionSubscriber *subscriber)
{
    ...
    FCFSVoteProtoHeader *proto_header;
    FCFSVoteProtoSessionPushRespBodyHeader *body_header;
    FCFSVoteProtoSessionPushRespBodyPart *body_part;
    char *p;
    char *end;
    int count;
    int result;
    ...
    proto_header = (FCFSVoteProtoHeader *)task->data;
    body_header = (FCFSVoteProtoSessionPushRespBodyHeader *)
        (proto_header + 1);
    p = (char *)(body_header + 1);
    end = task->data + task->size;
    count = 0;
    while (entry != NULL) {
        body_part = (FCFSVoteProtoSessionPushRespBodyPart *)p;
        if (p + sizeof(FCFSVoteProtoSessionPushRespBodyPart) +
            sizeof(FCFSVoteProtoSessionPushEntry) > end)
        {
            break;
        }
        ...
        if (entry->operation == FCFS_VOTE_SESSION_OP_TYPE_CREATE) {
            ...
            p += sizeof(FCFSVoteProtoSessionPushRespBodyPart) +
                sizeof(FCFSVoteProtoSessionPushEntry);
        } else {
            p += sizeof(FCFSVoteProtoSessionPushRespBodyPart);
        }
    ...
    SF_PROTO_SET_HEADER(proto_header, FCFS_VOTE_SERVICE_PROTO_SESSION_PUSH_REQ,
            task->length - sizeof(FCFSVoteProtoHeader));
...
int cluster_thread_loop_callback(struct nio_thread_data *thread_data)
{
    ...
    server_context = (VoteServerContext *)thread_data->arg;
...
void *cluster_alloc_thread_extra_data(const int thread_index)
{
    ...
    alloc_size = sizeof(VoteServerContext);
