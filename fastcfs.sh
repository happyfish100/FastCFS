...
validate_fastdir_params() {
  ...
  if [[ $dir_cluster_size -le 0 ]]; then
    echo "INFO:--dir-cluster-size not specified, would use default size: $DEFAULT_CLUSTER_SIZE"
  ...
  if [[ ${#dir_pathes[*]} -eq 0 ]]; then
    ...
    echo "ERROR:--dir-path must be one base path, or number of it equal to --dir-cluster-size!"
    exit 1
  fi
  ...
  if [[ ${#dir_hosts[*]} -eq 0 ]]; then
    ...
    echo "ERROR:--dir-host must be one IP, or number of it equal to --dir-cluster-size!"
    exit 1
  fi
  ...
  if [[ ${#dir_cluster_ports[*]} -eq 0 ]]; then
    ...
    echo "ERROR:--dir-cluster-port must be one port, or number of it equal to --dir-cluster-size!"
    exit 1
  fi
  ...
  if [[ ${#dir_service_ports[*]} -eq 0 ]]; then
    ...
    echo "ERROR:--dir-service-port must be one port, or number of it equal to --dir-cluster-size!"
    exit 1
  fi
  ...
  if [[ ${#dir_bind_addrs[*]} -eq 0 ]]; then
    ...
    echo "ERROR:--dir-bind-addr must be one IP, or number of it equal to --dir-cluster-size!"
    exit 1
  fi
}
...
validate_faststore_params() {
  ...
  if [[ $store_cluster_size -le 0 ]]; then
    echo "INFO:--store-cluster-size not specified, would use default size: $DEFAULT_CLUSTER_SIZE"
  ...
  if [[ ${#store_pathes[*]} -eq 0 ]]; then
    ...
    echo "ERROR:--store-path must be one base path, or number of it equal to --store-cluster-size!"
    exit 1
  fi
  ...
  if [[ ${#store_hosts[*]} -eq 0 ]]; then
    ...
    echo "ERROR:--store-host must be one IP, or number of it equal to --store-cluster-size!"
    exit 1
  fi
  ...
  if [[ ${#store_cluster_ports[*]} -eq 0 ]]; then
    ...
    echo "ERROR:--store-cluster-port must be one port, or number of it equal to --store-cluster-size!"
    exit 1
  fi
  ...
  if [[ ${#store_service_ports[*]} -eq 0 ]]; then
    ...
    echo "ERROR:--store-service-port must be one port, or number of it equal to --store-cluster-size!"
    exit 1
  fi
  ...
  if [[ ${#store_replica_ports[*]} -eq 0 ]]; then
    ...
    echo "ERROR:--store-replica-port must be one port, or number of it equal to --store-cluster-size!"
    exit 1
  fi
  ...
  if [[ ${#store_bind_addrs[*]} -eq 0 ]]; then
    ...
    echo "ERROR:--store-bind-addr must be one IP, or number of it equal to --store-cluster-size!"
    exit 1
  fi
