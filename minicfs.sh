#!/bin/bash
#
# minicfs install, config and start a local fastcfs cluster on Linux. minicfs's primary goals are to be the best shell for local fastcfs application development and to support all fastcfs features that fit.
# minicfs shell support commands
...
make_installs() {
  ...
  echo "INFO:Then, you can execute command [minicfs.sh config] next step."
}
